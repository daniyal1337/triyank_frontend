  ...
  return (
    <nav className="relative" style={{ backgroundColor: 'rgb(182, 165, 153)' }}>
              ...
              {navItems.map((item) => (
                <div key={item.name}>
                  {(item.name === "Western" || item.name === "Traditional" || item.name === "Leather") && item.hasSubcategories ? (
                    ...
                    <Link
                      to={item.href}
                      className={`flex items-center justify-between px-6 ${item.name === "Contact Us" ? "py-2.5" : "py-3.5"} text-foreground text-lg font-semibold bg-[#b6a599]/20 hover:bg-[#b6a599]/40 transition-colors`}
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      {item.name}
      ...
      <ShoppingBag 
        ...
        onViewWishlist={() => setOffCanvasType('favorites')}
        onViewCart={() => setOffCanvasType('cart')}
