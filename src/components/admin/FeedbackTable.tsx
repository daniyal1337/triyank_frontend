  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) {
          setError("Could not load feedback. Please verify GET /api/feedback is available.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  return (
    <div className="bg-white dark:bg-card rounded-xl border border-border shadow-sm">
      <div className="p-5 border-b border-border">
        ...
        <p className="text-sm text-muted-foreground">Customer feedback submissions</p>
      </div>
              ...
              <tbody>
                {items.map((it, idx) => (
                  <tr key={it.id || it._id || idx} className="border-b border-border last:border-b-0">
                    ...
                    <td className="py-3 pr-4 font-medium whitespace-nowrap">{it.rating}/5</td>
                    <td className="py-3 pr-4 text-muted-foreground min-w-[320px]">{it.feedback}</td>
