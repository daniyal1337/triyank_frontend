...
const CuratedForYou = () => {
  ...
  // Get a mix of bestseller and featured products for curation
  const curatedProducts = allProducts
    .filter((product) => product.isBestseller || product.isFeatured)
    .slice(0, 8);

  // If not enough curated products, add some random ones
  const displayProducts = curatedProducts.length >= 4 
    ? curatedProducts 
    : [...curatedProducts, ...allProducts.slice(0, 8 - curatedProducts.length)];
                    ...
                    >
                      <div className="relative aspect-[4/5] overflow-hidden mb-3 rounded-lg bg-muted">
                        ...
                        {product.isBestseller && !product.isNew && (
                          <span className="absolute top-3 left-3 bg-[#1a1410] text-white text-xs px-2 py-1 tracking-wider uppercase font-medium">
                            BESTSELLER
                          </span>
                        )}
